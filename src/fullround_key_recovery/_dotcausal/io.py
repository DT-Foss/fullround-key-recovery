...
class CausalWriter:
    ...
    def add_triplet(
        ...
        Set is_inferred=True to persist an already-derived (inferred) edge into
        the file, so a later reader can skip re-running inference (the
        inference becomes a materialized cache, not a per-load computation).

        `attrs` (v1.1) is an optional dict of TYPED relation fields — condition,
        population, effect_size, temporal, co_causes — carried verbatim into the
        msgpack triplet. Unknown to older readers (they ignore extra keys), so
        no format-version bump: a plain triplet just omits it.
        ...
        if attrs:
            triplet['attrs'] = {k: v for k, v in attrs.items() if v}
    ...
    def get_all_triplets(self, include_inferred: bool = True) -> List[Dict]:
        ...
        for t in self._triplets:
            row = {
                ...
                'is_inferred': bool(t.get('is_inferred', False)),
                'attrs': t.get('attrs', {}),
                's_idx': t['s_idx'],
                'm_idx': t['m_idx'],
                'o_idx': t['o_idx']
            }
            (materialized_inferred if row['is_inferred'] else explicit).append(row)

        if not include_inferred:
            return explicit

        # If the file already carries materialized inferred edges, use them —
        # inference was computed at build time and persisted, so we skip the
        # per-load 3-pass run entirely (the graph "stays inferenced").
        if materialized_inferred:
            return explicit + materialized_inferred
