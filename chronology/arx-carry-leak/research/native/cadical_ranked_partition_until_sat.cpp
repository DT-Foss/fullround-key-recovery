...
Arguments parse_arguments(int argc, char **argv) {
  ...
  if (result.cnf.empty() || result.mode.empty() ||
      result.configuration.empty() || result.seconds <= 0.0)
    throw std::runtime_error(
        "cnf, mode, configuration, and seconds are required");
  if (result.configuration != "default" && result.configuration != "sat" &&
      result.configuration != "unsat" && result.configuration != "plain")
    throw std::runtime_error("unsupported CaDiCaL configuration");
  if (result.assumption_one_literals.size() != 8)
    throw std::runtime_error(
        "exactly eight assumption-one-literals are required");
  if (result.model_one_literals.size() < 9 ||
      result.model_one_literals.size() > 32)
    throw std::runtime_error(
        "model-one-literals must contain 9 through 32 literals");
  ...
  if (assumption_variables.size() != 8 ||
      model_variables.size() != result.model_one_literals.size())
    throw std::runtime_error(
        "mapped variables must be distinct within each role");
  if (!std::includes(model_variables.begin(), model_variables.end(),
                     assumption_variables.begin(), assumption_variables.end()))
    throw std::runtime_error(
        "assumption variables must be contained in model variables");
  ...
  for (const std::string &cell : result.cell_order) {
    if (!is_binary_width(cell, 8))
      throw std::runtime_error(
          "cell order entries must be eight-bit binary");
...
int main(int argc, char **argv) {
  try {
    ...
    if (!solver.configure(arguments.configuration.c_str()) ||
        !solver.set("quiet", 1))
    ...
    for (int cell_index = 0; cell_index < arguments.max_cells; ++cell_index) {
      ...
      std::cout << "PARTITION_RESULT {\"mode\":\"" << arguments.mode
      ...
      std::cout << ",\"model_width\":" << arguments.model_one_literals.size()
    ...
    std::cerr << "PARTITION_ERROR " << error.what() << '\n';
    return 2;
  }
}
