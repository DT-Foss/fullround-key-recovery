...
inline ulong2 present128_update_key(
    ulong high64,
    ulong low64,
    uint round_index
) {
    // The 128-bit register is `high64 || low64`.  A rotate-left by 61
    // stays exact as two cross-lane shifts and needs no 128-bit scalar type.
    ulong next_high64 = (high64 << 61u) | (low64 >> 3u);
    ulong next_low64 = (low64 << 61u) | (high64 >> 3u);
    const uint top_nibble = uint(next_high64 >> 60u);
    const uint second_nibble = uint((next_high64 >> 56u) & 0xful);
    next_high64 = (next_high64 & 0x00fffffffffffffful)
        | (ulong(PRESENT_SBOX[top_nibble]) << 60u)
        | (ulong(PRESENT_SBOX[second_nibble]) << 56u);
    // The five-bit round counter is XORed into register bits 66..62.
    next_low64 ^= ulong(round_index & 0x3u) << 62u;
    next_high64 ^= ulong(round_index >> 2u);
    return ulong2(next_high64, next_low64);
}

inline ulong2 present128_encrypt_two(
    ulong2 states,
    uint candidate,
    constant SearchParams &params
) {
    ulong key_high64 = (ulong(params.key_high32) << 32u)
        | ulong(params.key_mid_high32);
    ulong key_low64 = (ulong(params.key_mid_low32) << 32u)
        | ulong(candidate);
    for (uint round_index = 1u; round_index <= 31u; ++round_index) {
        ...
        const ulong2 next_key = present128_update_key(
            key_high64, key_low64, round_index
        );
        key_high64 = next_key.x;
        key_low64 = next_key.y;
    }
...
kernel void present128_filter(
    ...
    const ulong2 output = present128_encrypt_two(
        ulong2(
...
kernel void present128_blocks(
    ...
    const uint candidate = params.first_candidate + gid;
    const ulong2 ciphertext = present128_encrypt_two(
        ulong2(
            (ulong(params.plaintext[0]) << 32u) | ulong(params.plaintext[1]),
            (ulong(params.plaintext[2]) << 32u) | ulong(params.plaintext[3])
        ),
...
private struct Configuration {
    ...
    let target: [UInt32]
    let control: [UInt32]
    let keyHigh32: UInt32
    let keyMidHigh32: UInt32
    let keyMidLow32: UInt32
}
...
private final class MetalPresent128Host {
    ...
    init() throws {
        ...
        guard let filter = library.makeFunction(name: "present128_filter"),
              let blocks = library.makeFunction(name: "present128_blocks"),
...
do {
    let host = try MetalPresent128Host()
    try emit([
        "op": "ready",
        "version": "present128-metal-native-v1",
        "metal": host.identity,
    ])
    ...
    while let line = readLine() {
        ...
        if operation == "configure" {
            let config = Configuration(
                ...
                keyHigh32: try uint32(
                    request["key_high32"], field: "key_high32"
                ),
                keyMidHigh32: try uint32(
                    request["key_mid_high32"], field: "key_mid_high32"
                ),
                keyMidLow32: try uint32(
                    request["key_mid_low32"], field: "key_mid_low32"
                )
            )
