...
inline uint aes_sub_word(uint word) {
    return (uint(AES_SBOX[(word >> 24u) & 0xffu]) << 24u)
        | (uint(AES_SBOX[(word >> 16u) & 0xffu]) << 16u)
        | (uint(AES_SBOX[(word >> 8u) & 0xffu]) << 8u)
        | uint(AES_SBOX[word & 0xffu]);
}
...
inline void aes256_encrypt(
    ...
    thread uint words[60] = {
        key_word0, key_word1, key_word2, key_word3,
        key_word4, key_word5, key_word6, candidate
    };
    for (uint index = 8u; index < 60u; ++index) {
        uint schedule = words[index - 1u];
        if ((index & 7u) == 0u) {
            schedule = aes_sub_rot_word(schedule, index / 8u - 1u);
        } else if ((index & 7u) == 4u) {
            schedule = aes_sub_word(schedule);
        }
        words[index] = words[index - 8u] ^ schedule;
    }
    aes_add_round_key(state, words);
    for (uint round = 1u; round <= 14u; ++round) {
        aes_sub_bytes(state);
        aes_shift_rows(state);
        if (round < 14u) {
            aes_mix_columns(state);
        }
        aes_add_round_key(state, words + 4u * round);
    }
}
...
private final class MetalAES256Host {
    ...
    init() throws {
        ...
        guard let filter = library.makeFunction(name: "aes256_filter"),
              let blocks = library.makeFunction(name: "aes256_blocks"),
...
do {
    let host = try MetalAES256Host()
    try emit([
        "op": "ready",
        "version": "aes256-fips197-metal-native-v1",
        "metal": host.identity,
    ])
    ...
    while let line = readLine() {
        ...
        if operation == "configure" {
            let config = Configuration(
                ...
                keyWords0To6: try wordArray(
                    request["key_words_0_to_6"], field: "key_words_0_to_6", count: 7
                )
            )
