...
constant uint SIMON_Z3_BITS[40] = {
    1u, 1u, 0u, 1u, 1u, 0u, 1u, 1u, 1u, 0u,
    1u, 0u, 1u, 1u, 0u, 0u, 0u, 1u, 1u, 0u,
    0u, 1u, 0u, 1u, 1u, 1u, 1u, 0u, 0u, 0u,
    0u, 0u, 0u, 1u, 0u, 0u, 1u, 0u, 0u, 0u,
};

inline void simon64_128_round_keys(
    thread uint *round_keys,
    uint candidate,
    constant SearchParams &params
) {
    round_keys[0] = candidate;
    round_keys[1] = params.key1;
    round_keys[2] = params.key2;
    round_keys[3] = params.key3;
    for (uint round = 4u; round < 44u; ++round) {
        uint temporary = ror32(round_keys[round - 1u], 3u);
        temporary ^= round_keys[round - 3u];
        temporary ^= ror32(temporary, 1u);
        round_keys[round] = round_keys[round - 4u]
            ^ temporary
            ^ 0xfffffffcu
            ^ SIMON_Z3_BITS[round - 4u];
    }
}

inline uint2 simon64_128_encrypt(uint2 state, thread const uint *round_keys) {
    uint x = state.x;
    uint y = state.y;
    for (uint round = 0u; round < 44u; ++round) {
        const uint function = (
            (rol32(x, 1u) & rol32(x, 8u)) ^ rol32(x, 2u)
        );
        const uint next_x = y ^ function ^ round_keys[round];
        y = x;
        x = next_x;
    }
    return uint2(x, y);
...
kernel void simon64_128_filter(
    ...
    thread uint round_keys[44];
    simon64_128_round_keys(round_keys, candidate, params);
    ...
    for (uint block = 0u; block < 2u; ++block) {
        ...
        const uint2 output = simon64_128_encrypt(
...
kernel void simon64_128_blocks(
    constant SearchParams &params [[buffer(0)]],
    ...
    for (uint block = 0u; block < 2u; ++block) {
        const uint offset = block * 2u;
        const uint2 ciphertext = simon64_128_encrypt(
            uint2(params.plaintext[offset], params.plaintext[offset + 1u]),
            round_keys
        );
...
private final class MetalSimon64128Host {
    ...
    init() throws {
        ...
        guard let filter = library.makeFunction(name: "simon64_128_filter"),
              let blocks = library.makeFunction(name: "simon64_128_blocks"),
...
do {
    let host = try MetalSimon64128Host()
    try emit([
        "op": "ready",
        "version": "simon64-128-metal-native-v1",
        "metal": host.identity,
    ])
