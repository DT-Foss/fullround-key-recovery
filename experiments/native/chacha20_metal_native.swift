...
kernel void chacha20_filter(
    ...
    chacha20_block(state, params, candidate);
    if (state[0] == params.target[0] && state[1] == params.target[1]) {
        ...
        if (offset < params.result_capacity) {
            factual_candidates[offset] = candidate;
        }
    }
    if (state[0] == params.control[0] && state[1] == params.control[1]) {
        ...
        if (offset < params.result_capacity) {
            control_candidates[offset] = candidate;
        }
    }
}

kernel void chacha20_blocks(
    constant SearchParams &params [[buffer(0)]],
    ...
    thread uint state[16];
    chacha20_block(state, params, params.first_candidate + gid);
    for (uint word = 0u; word < 16u; ++word) {
        output[gid * 16u + word] = state[word];
    }
}
"""#
...
private func parameterWords(
    ...
    return config.initial + config.target + config.control + [first, count, capacity]
}

private final class MetalChaCha20Host {
    ...
    init() throws {
        ...
        guard let filter = library.makeFunction(name: "chacha20_filter"),
              let blocks = library.makeFunction(name: "chacha20_blocks"),
...
do {
    let host = try MetalChaCha20Host()
    try emit(["op": "ready", "version": "chacha20-metal-native-v1", "metal": host.identity])
    ...
    while let line = readLine() {
        ...
        if operation == "configure" {
            let config = Configuration(
                initial: try uint32Array(request["initial"], field: "initial", count: 16),
