...
inline void speck64_128_round_keys(
    thread uint *round_keys,
    uint candidate,
    constant SearchParams &params
) {
    thread uint l_words[29];
    round_keys[0] = candidate;
    l_words[0] = params.key1;
    l_words[1] = params.key2;
    l_words[2] = params.key3;
    for (uint round = 0u; round < 26u; ++round) {
        const uint next_l = (
            (ror32(l_words[round], 8u) + round_keys[round]) ^ round
        );
        l_words[round + 3u] = next_l;
        round_keys[round + 1u] = rol32(round_keys[round], 3u) ^ next_l;
    }
}

inline uint2 speck64_128_encrypt(uint2 state, thread const uint *round_keys) {
    uint x = state.x;
    uint y = state.y;
    for (uint round = 0u; round < 27u; ++round) {
        x = (ror32(x, 8u) + y) ^ round_keys[round];
        y = rol32(y, 3u) ^ x;
    }
    return uint2(x, y);
}
...
kernel void speck64_128_filter(
    ...
    thread uint round_keys[27];
    speck64_128_round_keys(round_keys, candidate, params);
    ...
    for (uint block = 0u; block < 2u; ++block) {
        ...
        const uint2 output = speck64_128_encrypt(
...
kernel void speck64_128_blocks(
    constant SearchParams &params [[buffer(0)]],
    ...
    for (uint block = 0u; block < 2u; ++block) {
        const uint offset = block * 2u;
        const uint2 ciphertext = speck64_128_encrypt(
            uint2(params.plaintext[offset], params.plaintext[offset + 1u]),
            round_keys
        );
...
private final class MetalSpeck64128Host {
    ...
    init() throws {
        ...
        guard let filter = library.makeFunction(name: "speck64_128_filter"),
              let blocks = library.makeFunction(name: "speck64_128_blocks"),
...
do {
    let host = try MetalSpeck64128Host()
    try emit([
        "op": "ready",
        "version": "speck64-128-metal-native-v1",
        "metal": host.identity,
    ])
