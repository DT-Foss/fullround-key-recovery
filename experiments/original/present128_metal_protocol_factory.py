#!/usr/bin/env python3
"""Freeze P128R1 only after retained P128Q1 selects a safe PRESENT-128 width.
...
from arx_carry_leak.present128_reference import ROUNDS, encrypt_int, key_parts_to_int, key_schedule

ATTEMPT_ID = "P128R1"
QUALIFICATION_ATTEMPT_ID = "P128Q1"
QUALIFICATION_SCHEMA = "present128-metal-qualification-v1"
QUALIFICATION_FILENAME = "present128_metal_qualification_v1.json"
...
def _width_parameters(width: int) -> dict[str, int]:
    ...
    return {
        ...
        "mid_low_known_mask": mid_low_known_mask,
    }


def _known_material(width: int) -> tuple[int, int, list[int], str, str, str]:
    params = _width_parameters(width)
    label = f"present128/p128r1/fullround/w{width}/known-material/v1"
    raw = hashlib.shake_256(label.encode()).digest(32)
    mid_low32_known = int.from_bytes(raw[:4], "big") & params["mid_low_known_mask"]
    ...
    if len(blocks) != PLAINTEXT_BLOCKS:
        raise RuntimeError("P128R1 deterministic plaintext blocks are not distinct")
    return (
        mid_low32_known,
        high64,
        plaintext_words,
...
def _target_words(
    ...
    *,
    ...
    if assignment < 0 or assignment >= params["logical_candidates"]:
        raise ValueError(f"P128R1 assignment is outside the W{width} domain")
    candidate = assignment & 0xFFFFFFFF
    outer = assignment >> 32
    mid_low32 = mid_low32_known | outer
    round_keys = key_schedule(key_parts_to_int(high64, mid_low32, candidate))
    output: list[int] = []
    for offset in range(0, len(plaintext_words), 2):
...
def _qualification_gate(payload: dict[str, Any]) -> int:
    ...
    if not isinstance(width, int):
        raise RuntimeError("P128Q1 qualification did not select an integer width")
    ...
    if (
        ...
        or payload.get("evidence_stage") != "PRESENT128_METAL_PRE_TARGET_QUALIFICATION"
        ...
        raise RuntimeError("P128Q1 qualification semantic or two-hour gate differs")
    return width
...
def build_protocol(
    *, qualification: Path, native_source: Path, reference_source: Path
    ...
    """Generate one fresh public P128R1 challenge and discard its assignment."""
    ...
    if (
        ...
        raise RuntimeError("P128Q1 qualification and PRESENT-128 native hashes differ")
    ...
    target_words = _target_words(
        unknown_assignment,
        width=width,
        mid_low32_known=mid_low32_known,
        high64=high64,
    ...
    public_challenge = {
        ...
        "known_mid_low32": mid_low32_known,
        "known_mid_low32_mask": params["mid_low_known_mask"],
        "known_high64": high64,
        "unknown_low32_bits": 32,
        "unknown_mid_low32_low_bits": outer_bits,
        "unknown_assignment_bits": width,
        "known_master_key_bits": params["known_key_bits"],
        "candidate_encoding": (
            f"assignment=(mid_low32_low{outer_bits}<<32)|low32"
            if outer_bits
            else "assignment=low32"
        ),
        "unknown_assignment_included": False,
        "unknown_low32_included": False,
        "unknown_mid_low32_low_bits_included": False,
    ...
    execution_plan = {
        ...
        "outer_mid_low32_bit_count": outer_bits,
        "outer_mid_low32_slice_count": params["outer_slices"],
    ...
    return {
        "schema": f"present128-metal-width{width}-recovery-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "frozen_before_any_P128R1_candidate_execution",
        "primary_sources": {
            "algorithm_specification": (
                "https://www.iacr.org/archive/ches2007/47270450/47270450.pdf"
            ),
            "official_zero_vector": "PRESENT-128_E_0_0=96db702a2e6900af",
            "variant_standard": "ISO/IEC 29167-11 PRESENT-128",
            "orientation_trace": "local non-palindromic schedule sentinel",
        },
        "anchors": {
            "qualification": {
        ...
        "required_validation_gates": {
            "pre_target_official_and_local_KATs_passed": True,
            ...
            "candidate_execution_against_public_P128R1_target_before_freeze": False,
        ...
        "information_boundary": {
            ...
            "unknown_assignment_available_to_runner_before_execution": False,
            "P128R1_candidate_outcomes_used_before_protocol_freeze": False,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    output = args.output or (
        research_root / "configs" / f"present128_metal_width{width}_recovery_v1.json"
    )
    if output.exists():
        raise FileExistsError(f"P128R1 protocol already exists: {output}")
