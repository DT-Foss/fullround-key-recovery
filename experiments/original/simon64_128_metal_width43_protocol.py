#!/usr/bin/env python3
"""Create the one-shot pre-execution protocol for A246 SIMON64/128 W43."""
...
ATTEMPT_ID = "A246"
QUALIFICATION_ATTEMPT_ID = "A245"
SCHEMA = "simon64-128-metal-width43-recovery-protocol-v1"
UNKNOWN_BITS = 43
OUTER_BITS = 11
...
KNOWN_MATERIAL_LABEL = "simon64-128/a246/fullround/w43/known-material/v1"
...
def _known_material() -> tuple[int, int, int, list[int], str]:
    ...
    if len(blocks) != PLAINTEXT_BLOCKS:
        raise RuntimeError("A246 deterministic plaintext blocks are not distinct")
    return key1_known_upper21, key2, key3, plaintext_words, _sha256(raw)


def _target_words(
    assignment: int,
    key1_known_upper21: int,
    key2: int,
    key3: int,
    plaintext_words: list[int],
) -> list[int]:
    if assignment < 0 or assignment >= LOGICAL_CANDIDATES:
        raise ValueError("A246 assignment is outside the W43 domain")
    inner = assignment & 0xFFFFFFFF
    outer = assignment >> 32
    master_key = [inner, key1_known_upper21 | outer, key2, key3]
...
def _qualification_gate(payload: dict[str, Any]) -> None:
    ...
    if (
        payload.get("schema") != "simon64-128-metal-qualification-v1"
        ...
        raise RuntimeError("A246 qualification semantic or two-hour gate differs")


def build_protocol(*, qualification: Path, native_source: Path) -> dict[str, Any]:
    if _file_sha256(qualification) != QUALIFICATION_SHA256:
        raise RuntimeError("A246 qualification anchor hash differs")
    ...
    if _file_sha256(native_source) != NATIVE_SOURCE_SHA256:
        raise RuntimeError("A246 native source anchor hash differs")
    ...
    public_challenge = {
        ...
        "known_key1_upper21": key1_known_upper21,
        "known_key2": key2,
        "known_key3": key3,
        ...
        "candidate_encoding": "assignment=(key1_low11<<32)|key0",
        "unknown_assignment_included": False,
        "unknown_key0_included": False,
        "unknown_key1_low11_included": False,
    ...
    return {
        ...
        "protocol_state": "frozen_before_any_A246_candidate_execution",
        "primary_sources": {
            "algorithm_and_SIMON64_128_test_vector": (
        ...
        "prospective_prediction": {
            "claim_type": "fresh_fullround_43_bit_residual_key_recovery",
        ...
        "required_validation_gates": {
            ...
            "pre_target_width43_two_hour_gate_passed": True,
            "candidate_execution_against_public_A246_target_before_freeze": False,
            "all_2^43_assignments_must_execute": True,
        ...
        "information_boundary": {
            ...
            "unknown_assignment_available_to_runner_before_execution": False,
            "A246_candidate_outcomes_used_before_protocol_freeze": False,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=(
            research_root / "configs" / "simon64_128_metal_width43_recovery_v1.json"
        ),
    )
    parser.add_argument(
        "--qualification",
    ...
    if args.output.exists():
        raise FileExistsError(f"A246 protocol already exists: {args.output}")
        ...
        json.dumps(
            {
                ...
                "width43_launch_gate": protocol["qualification_launch_gate"],
            },
            indent=2,
            sort_keys=True,
        )
