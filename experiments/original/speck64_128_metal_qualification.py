#!/usr/bin/env python3
"""Qualify and benchmark the full-round Speck64/128 Metal key enumerator.
...
ATTEMPT_ID = "A243"
SCHEMA = "speck64-128-metal-qualification-v1"
NATIVE_SOURCE_FILENAME = "speck64_128_metal_native.swift"
NATIVE_VERSION = "speck64-128-metal-native-v1"
PLAINTEXT_BLOCKS = 2
WORDS_PER_BLOCK = 2
...
REQUIRED_CANDIDATES_PER_SECOND = (1 << TARGET_WIDTH) / MAX_COMPLETE_DOMAIN_SECONDS
VARIANT = SPECK_VARIANTS["speck64_128"]
OFFICIAL_KAT_MASTER_KEY = [0x03020100, 0x0B0A0908, 0x13121110, 0x1B1A1918]
OFFICIAL_KAT_PLAINTEXT = (0x3B726574, 0x7475432D)
OFFICIAL_KAT_CIPHERTEXT = (0x8C6FA548, 0x454E028B)
QUALIFICATION_PLAINTEXT = np.array(
    [0x3B726574, 0x7475432D, 0x00000000, 0x00000000], dtype=np.uint32
)
...
def _compile_native(build_dir: Path, swiftc: str) -> tuple[Path, dict[str, Any]]:
    ...
    output = build_dir / f"speck64_128_metal_{source_sha[:16]}"
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Speck64/128 Swift/Metal host compilation failed: " + result.stderr.strip()
        )
    temporary.replace(output)
    if not output.is_file() or output.stat().st_size == 0:
        raise RuntimeError("Speck64/128 Swift/Metal host build produced no executable")
...
def _candidate_words(candidate: int, key1: int, key2: int, key3: int) -> list[int]:
    ...
    if any(word < 0 or word > 0xFFFFFFFF for word in words):
        raise ValueError("all Speck64/128 key words must fit in uint32")
    return words
...
def _scalar_outputs(
    ...
    round_keys = speck_round_keys(
        VARIANT, _candidate_words(candidate, key1, key2, key3), VARIANT.full_rounds
    )
...
class MetalSpeck64128Host:
    def __init__(self, executable: Path):
        ...
        if (
            ...
            raise RuntimeError("Speck64/128 Metal host identity gate failed")
        self.identity = ready
    ...
    def _read(self) -> dict[str, Any]:
        ...
        if not line:
            ...
            raise RuntimeError("Speck64/128 Metal host closed unexpectedly: " + diagnostics)
        value = json.loads(line)
        if not isinstance(value, dict):
            raise RuntimeError("Speck64/128 Metal host returned a non-object")
        return value

    def _request(self, value: dict[str, Any]) -> dict[str, Any]:
        if self.process.poll() is not None:
            raise RuntimeError("Speck64/128 Metal host is not running")
    ...
    def configure(
        ...
        *,
        ...
        if (
            ...
            raise RuntimeError("Speck64/128 Metal configuration gate failed")
    ...
    def blocks(self, first: int, count: int) -> np.ndarray:
        ...
        if (
            ...
            raise RuntimeError("Speck64/128 Metal block response gate failed")
    ...
    def filter(self, first: int, count: int) -> dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError("Speck64/128 Metal filter response gate failed")
        return response
    ...
    def close(self, *, force: bool = False) -> None:
        ...
        if not force and code != 0:
            assert self.process.stderr is not None
            raise RuntimeError(
                "Speck64/128 Metal host exit failed: " + self.process.stderr.read()
            )


def _kat_gate(host: MetalSpeck64128Host) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("Speck64/128 official KAT/Metal equivalence gate failed")
    return {
...
def _cross_gate(host: MetalSpeck64128Host) -> dict[str, Any]:
    first = 123_456_789
    count = 256
    offset = 73
    key1 = 0x89ABCDEF
    ...
    if (
        ...
        raise RuntimeError("Speck64/128 Metal/scalar cross gate failed")
    return {
...
def _boundary_gate(host: MetalSpeck64128Host) -> dict[str, Any]:
    ...
    for first, count in intervals:
        ...
        if result["factual"] != expected or result["control"] != []:
            raise RuntimeError("Speck64/128 Metal boundary filter gate failed")
        rows.append(
            {
                "first_candidate": first,
                "candidate_count": count,
    ...
    *,
    ...
    for _ in range(repeats):
        ...
        if gpu_seconds <= 0.0 or wall_seconds <= 0.0:
            raise RuntimeError("Speck64/128 Metal benchmark returned zero elapsed time")
        samples.append(
            {
...
def run(
    *,
    ...
    host = MetalSpeck64128Host(executable)
    try:
        kat = _kat_gate(host)
    ...
    projected_width44_seconds = (1 << TARGET_WIDTH) / minimum_throughput
    ...
    launch_gate = {
        ...
        "projected_width44_seconds_at_observed_minimum": projected_width44_seconds,
        "highest_safe_integer_width_at_observed_minimum": highest_safe_width,
        "selected_width": TARGET_WIDTH if highest_safe_width >= TARGET_WIDTH else highest_safe_width,
        "width44_under_two_hours": projected_width44_seconds <= MAX_COMPLETE_DOMAIN_SECONDS,
    }
    launch_gate["full_domain_launch_authorized"] = bool(
        launch_gate["width44_under_two_hours"]
    )
    payload = {
        "schema": SCHEMA,
        "attempt_id": ATTEMPT_ID,
        "evidence_stage": "SPECK64_128_METAL_PRE_TARGET_QUALIFICATION",
        "scope": (
            "Full 27-round Speck64/128 implementation qualification and volatile "
        ...
        "cipher": {
            ...
            "master_key_bits": 128,
            "rounds": 27,
            "candidate_encoding": "candidate=k0; configured_key1=l0, key2=l1, key3=l2",
    ...
    if reopened != payload:
        raise RuntimeError("Speck64/128 qualification artifact reopen gate failed")
    return {
        "output": str(output),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=research_root / "results" / "v1" / "speck64_128_metal_qualification_v1.json",
    )
    parser.add_argument(
        "--build-dir",
        type=Path,
        default=research_root / "build" / "speck64_128_metal",
    )
