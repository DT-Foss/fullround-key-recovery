#!/usr/bin/env python3
"""Qualify and benchmark the full-round RC5-32/12/16 Metal key enumerator.
...
from arx_carry_leak.rc5_reference import (
    ROUNDS,
    encrypt_words,
    expand_key_words,
    verify_rfc2040_derived_r12,
    verify_rivest_kats,
)

ATTEMPT_ID = "A247"
SCHEMA = "rc5-32-12-16-metal-qualification-v1"
NATIVE_SOURCE_FILENAME = "rc5_32_12_16_metal_native.swift"
NATIVE_VERSION = "rc5-32-12-16-metal-native-v1"
PLAINTEXT_BLOCKS = 2
WORDS_PER_BLOCK = 2
...
KEY_WORDS = 4
FULL_ROUNDS = ROUNDS
OFFICIAL_KAT_MASTER_KEY = [0x19465F91, 0x51B241BE, 0x01A55563, 0x91CEA910]
OFFICIAL_KAT_PLAINTEXT = (0xEEDBA521, 0x6D8F4B15)
OFFICIAL_KAT_CIPHERTEXT = (0xAC13C0F7, 0x52892B5B)
QUALIFICATION_PLAINTEXT = np.array(
    [*OFFICIAL_KAT_PLAINTEXT, 0x12345678, 0x9ABCDEF0], dtype=np.uint32
)
...
def _compile_native(build_dir: Path, swiftc: str) -> tuple[Path, dict[str, Any]]:
    ...
    output = build_dir / f"rc5_32_12_16_metal_{source_sha[:16]}"
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "RC5-32/12/16 Swift/Metal host compilation failed: " + result.stderr.strip()
        )
    temporary.replace(output)
    if not output.is_file() or output.stat().st_size == 0:
        raise RuntimeError("RC5-32/12/16 Swift/Metal host build produced no executable")
...
def _candidate_words(candidate: int, key1: int, key2: int, key3: int) -> list[int]:
    ...
    if any(word < 0 or word > 0xFFFFFFFF for word in words):
        raise ValueError("all RC5-32/12/16 key words must fit in uint32")
    return words
...
def _scalar_outputs(
    ...
    subkeys = expand_key_words(_candidate_words(candidate, key1, key2, key3))
...
class MetalRC5321216Host:
    def __init__(self, executable: Path):
        ...
        if (
            ...
            raise RuntimeError("RC5-32/12/16 Metal host identity gate failed")
        self.identity = ready
    ...
    def _read(self) -> dict[str, Any]:
        ...
        if not line:
            ...
            raise RuntimeError("RC5-32/12/16 Metal host closed unexpectedly: " + diagnostics)
        value = json.loads(line)
        if not isinstance(value, dict):
            raise RuntimeError("RC5-32/12/16 Metal host returned a non-object")
        return value

    def _request(self, value: dict[str, Any]) -> dict[str, Any]:
        if self.process.poll() is not None:
            raise RuntimeError("RC5-32/12/16 Metal host is not running")
    ...
    def configure(
        ...
        *,
        ...
        if (
            ...
            raise RuntimeError("RC5-32/12/16 Metal configuration gate failed")
    ...
    def blocks(self, first: int, count: int) -> np.ndarray:
        ...
        if (
            ...
            raise RuntimeError("RC5-32/12/16 Metal block response gate failed")
    ...
    def filter(self, first: int, count: int) -> dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError("RC5-32/12/16 Metal filter response gate failed")
        return response
    ...
    def close(self, *, force: bool = False) -> None:
        ...
        if not force and code != 0:
            assert self.process.stderr is not None
            raise RuntimeError(
                "RC5-32/12/16 Metal host exit failed: " + self.process.stderr.read()
            )


def _kat_gate(host: MetalRC5321216Host) -> dict[str, Any]:
    rivest_kats = verify_rivest_kats()
    rfc2040_derived = verify_rfc2040_derived_r12()
    if (
        len(rivest_kats) != 5
        or not all(row["pass"] is True for row in rivest_kats)
        or rfc2040_derived["pass"] is not True
    ):
        raise RuntimeError("RC5-32/12/16 scalar provenance/KAT gate failed")
    ...
    if (
        ...
        raise RuntimeError("RC5-32/12/16 provenance KAT/Metal equivalence gate failed")
    return {
        "rivest_original_paper_scalar_vectors": rivest_kats,
        "rfc2040_derived_r12_raw_block_vector": rfc2040_derived,
        "master_key_words_little_endian": OFFICIAL_KAT_MASTER_KEY,
        ...
        "candidate_word_k0": candidate,
        "rfc2040_byte_convention": "four little-endian uint32 key words and two little-endian uint32 block words",
    }


def _cross_gate(host: MetalRC5321216Host) -> dict[str, Any]:
    first = 123_456_789
    count = 256
    offset = 73
    key1 = 0x89ABCDEF
    ...
    if (
        ...
        raise RuntimeError("RC5-32/12/16 Metal/scalar cross gate failed")
    return {
...
def _boundary_gate(host: MetalRC5321216Host) -> dict[str, Any]:
    ...
    for first, count in intervals:
        ...
        if result["factual"] != expected or result["control"] != []:
            raise RuntimeError("RC5-32/12/16 Metal boundary filter gate failed")
        rows.append(
            {
                "first_candidate": first,
                "candidate_count": count,
    ...
    *,
    ...
    for _ in range(repeats):
        ...
        if gpu_seconds <= 0.0 or wall_seconds <= 0.0:
            raise RuntimeError("RC5-32/12/16 Metal benchmark returned zero elapsed time")
        samples.append(
            {
...
def run(
    *,
    ...
    host = MetalRC5321216Host(executable)
    try:
        kat = _kat_gate(host)
    ...
    payload = {
        ...
        "evidence_stage": "RC5_32_12_16_METAL_PRE_TARGET_QUALIFICATION",
        "scope": (
            "Full 12-round RC5-32/12/16 implementation qualification and volatile "
            "throughput measurement; no production recovery target was selected or run."
        ),
        "cipher": {
            "variant": "RC5-32/12/16",
            "block_bits": 64,
            "master_key_bits": 128,
            "rounds": FULL_ROUNDS,
            "key_schedule": "RFC_2040_P32_Q32_26_subkeys_78_mix_steps",
            "references": [
                "Rivest_original_RC5_paper_examples",
                "RFC_2040_byte_conventions_and_CBC_vector",
            ],
    ...
    if reopened != payload:
        raise RuntimeError("RC5-32/12/16 qualification artifact reopen gate failed")
    return {
        "output": str(output),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=research_root / "results" / "v1" / "rc5_32_12_16_metal_qualification_v1.json",
    )
    parser.add_argument(
        "--build-dir",
        type=Path,
        default=research_root / "build" / "rc5_32_12_16_metal",
    )
