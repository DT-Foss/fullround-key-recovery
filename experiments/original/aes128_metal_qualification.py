#!/usr/bin/env python3
"""Pre-target qualification for the prospective FIPS-197 AES-128 factory.
...
from arx_carry_leak.live_casi_v091.ciphers import _aes_ecb_batch

SCHEMA = "aes128-fips197-metal-qualification-v1"
STAGE = "AES128_FIPS197_METAL_PRE_TARGET_QUALIFICATION"
NATIVE_SOURCE_FILENAME = "aes128_metal_native.swift"
REFERENCE_SOURCE_FILENAME = "aes128_reference.py"
NATIVE_VERSION = "aes128-fips197-metal-native-v1"
PLAINTEXT_BLOCKS = 2
...
QUALIFICATION_KNOWN_KEY = bytes.fromhex("00112233445566778899aabb00000000")
METAL_EVIDENCE_LEDGER_SCHEMA = "aes128-fips197-metal-evidence-ledger-v1"
METAL_EVIDENCE_PRODUCER = (
    "aes128_metal_qualification.run_metal_qualification"
)
...
def independent_numpy_source_path() -> Path:
    ...
    observed = Path(inspect.getsourcefile(_aes_ecb_batch) or "").resolve()
    if observed != expected or not observed.is_file():
        raise RuntimeError("AES-128 independent NumPy implementation path differs")
    return observed
...
def independent_numpy_encrypt(key: bytes, plaintext: bytes) -> bytes:
    ...
    blocks = np.frombuffer(raw, dtype=np.uint8).reshape(-1, 16).copy()
    return _aes_ecb_batch(key_array, blocks, rounds=ROUNDS).tobytes()
...
def _cpu_kat_gate() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("AES-128 FIPS-197 CPU KAT gate failed")
    return {
...
def _cpu_mapping_gate() -> dict[str, Any]:
    seed_key = bytes.fromhex("00112233445566778899aabbccddeeff")
    cases = (
        (32, 0x00000000),
        (32, 0x0000FFFF),
        (32, 0x80000000),
    ...
    for width, assignment in cases:
        ...
        if (
            scalar != independent
            or words[3] != (assignment & 0xFFFFFFFF)
            or words[2] & ((1 << (width - 32)) - 1) != assignment >> 32
        ):
            raise RuntimeError(f"AES-128 residual mapping gate failed for width {width}")
        rows.append(
            {
                "width": width,
    ...
    return {
        "candidate_encoding": (
            ...
            "inner uint32 is FIPS key word3 (bytes 12..15, big-endian); outer bits "
            "are the low bits of FIPS key word2"
        ),
        "assignment_bit0": "key_byte_15_bit_0",
        "inner_candidate_byte_mapping": {
            "key_byte_12": "candidate_bits_31_to_24",
            "key_byte_13": "candidate_bits_23_to_16",
            "key_byte_14": "candidate_bits_15_to_8",
            "key_byte_15": "candidate_bits_7_to_0",
        },
        "rows": rows,
...
def run_cpu_qualification() -> dict[str, Any]:
    ...
    payload = {
        "schema": SCHEMA,
        "evidence_stage": "AES128_FIPS197_CPU_ONLY_FACTORY_PREPARATION",
        "algorithm": {
            "name": "AES-128",
            "standard": "FIPS_197",
            "rounds": ROUNDS,
...
def _compile_native(build_dir: Path, swiftc: str) -> tuple[Path, dict[str, Any]]:
    ...
    output = build_dir / f"aes128_metal_{source_sha256[:16]}"
    ...
    if result.returncode != 0:
        raise RuntimeError("AES-128 Swift/Metal host compilation failed: " + result.stderr.strip())
...
class MetalAES128Host:
    ...
    def __init__(
        ...
        try:
            ...
            if (
                ...
                or metal.get("candidate_maps_to_key_bytes_12_through_15_big_endian")
                is not True
            ):
                raise RuntimeError("AES-128 Metal host identity gate failed")
    ...
    def _read(self) -> dict[str, Any]:
        ...
        if self.deadline_monotonic is not None:
            ...
            if remaining <= 0:
                self.close(force=True)
                raise TimeoutError("AES-128 Metal qualification wall cap expired")
            readable, _, _ = select.select([self.process.stdout], [], [], remaining)
            if not readable:
                self.close(force=True)
                raise TimeoutError("AES-128 Metal qualification exceeded its wall cap")
        line = self.process.stdout.readline()
        if not line:
            assert self.process.stderr is not None
            raise RuntimeError(
                "AES-128 Metal host closed unexpectedly: "
                + self.process.stderr.read().strip()
            )
        value = json.loads(line)
        if not isinstance(value, dict):
            raise RuntimeError("AES-128 Metal host returned a non-object")
        if "gpu_seconds" in value:
            elapsed = float(value["gpu_seconds"])
            if elapsed < 0:
                raise RuntimeError("AES-128 Metal host returned negative GPU time")
            self.total_gpu_seconds += elapsed
        return value

    def _request(self, value: dict[str, Any]) -> dict[str, Any]:
        if self.process.poll() is not None:
            raise RuntimeError("AES-128 Metal host is not running")
    ...
    def configure(
        ...
        *,
        ...
        target: bytes,
        control: bytes,
        key_words_0_to_2: Sequence[int],
    ) -> None:
        if len(plaintext) != FILTER_BYTES or len(target) != FILTER_BYTES:
            raise ValueError("AES-128 native relation requires exactly two blocks")
        if len(control) != FILTER_BYTES:
            raise ValueError("AES-128 control must contain exactly two blocks")
        words = [int(value) for value in key_words_0_to_2]
        if len(words) != 3 or any(value < 0 or value > 0xFFFFFFFF for value in words):
            raise ValueError("AES-128 key_words_0_to_2 must be three uint32 values")
        ...
        if (
            ...
            or response.get("candidate_key_word") != 3
        ):
            raise RuntimeError("AES-128 Metal configuration gate failed")
    ...
    def blocks(self, first: int, count: int) -> list[bytes]:
        ...
        if (
            ...
            raise RuntimeError("AES-128 Metal blocks response gate failed")
        return [
    ...
    def filter(self, first: int, count: int) -> dict[str, Any]:
        ...
        if (
            ...
            raise RuntimeError("AES-128 Metal filter response gate failed")
        return response
    ...
    def close(self, *, force: bool = False) -> None:
        ...
        if not force and code != 0:
            raise RuntimeError("AES-128 Metal host exit failed")


def _metal_kat_and_cross_gate(host: MetalAES128Host) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    for vector in FIPS197_KATS:
        ...
        host.configure(
            ...
            key_words_0_to_2=words[:3],
        )
        outputs = host.blocks(words[3], 1)
        filtered = host.filter(words[3], 1)
        if outputs != [target] or filtered["factual"] != [words[3]] or filtered["control"]:
            raise RuntimeError(f"AES-128 Metal KAT failed for {vector.name}")
        rows.append(
            {
                "name": vector.name,
    ...
    expected = [
        encrypt_blocks(
            QUALIFICATION_KNOWN_KEY[:12] + (first + index).to_bytes(4, "big"),
    ...
    host.configure(
        ...
        target=target,
        control=_one_bit_control(target),
        key_words_0_to_2=known_words[:3],
    )
    ...
    if observed != expected or filtered["factual"] != [first + offset] or filtered["control"]:
        raise RuntimeError("AES-128 CPU/Metal cross-implementation gate failed")
    return {
...
def _metal_mapping_gate(host: MetalAES128Host) -> dict[str, Any]:
    ...
    for outer, first, count, offset, label in METAL_MAPPING_CASES:
        ...
        host.configure(
            ...
            target=target,
            control=_one_bit_control(target),
            key_words_0_to_2=key_words_big_endian(known_prefix + bytes(4))[:3],
        )
        observed = host.blocks(first, count)
        filtered = host.filter(first, count)
        candidate = first + offset
        if observed != expected or filtered["factual"] != [candidate] or filtered["control"]:
            raise RuntimeError(f"AES-128 Metal mapping gate failed at {label}")
        rows.append(
            {
                "label": label,
                "outer_word2": outer,
                "target_inner_word3": candidate,
...
def _benchmark(
    host: MetalAES128Host, *, candidate_count: int, repeats: int
) -> dict[str, Any]:
    ...
    target = encrypt_blocks(
        QUALIFICATION_KNOWN_KEY[:12] + target_candidate.to_bytes(4, "big"),
        QUALIFICATION_PLAINTEXT,
    )
    host.configure(
        plaintext=QUALIFICATION_PLAINTEXT,
    ...
    for _ in range(repeats):
        ...
        if gpu_seconds <= 0 or wall_seconds <= 0:
            raise RuntimeError("AES-128 benchmark returned zero elapsed time")
        if response["factual"] != [target_candidate] or response["control"]:
            raise RuntimeError("AES-128 benchmark correctness gate failed")
        samples.append(
            {
...
def _expected_metal_cross_evidence() -> dict[str, Any]:
    ...
    expected = [
        encrypt_blocks(
            QUALIFICATION_KNOWN_KEY[:12] + candidate.to_bytes(4, "big"),
            QUALIFICATION_PLAINTEXT,
        )
...
def validate_metal_evidence_ledger(payload: dict[str, Any]) -> str:
    ...
    if (
        ...
        raise RuntimeError("AES-128 Metal evidence ledger hash gate failed")
    ...
    if (
        ...
        raise RuntimeError("AES-128 Metal evidence ledger binding gate failed")
    ...
    if (
        ...
        or metal.get("shader_runtime_compiled") is not True
        or metal.get("fips197_external_byte_order") is not True
        or metal.get("candidate_maps_to_key_bytes_12_through_15_big_endian") is not True
        or cross != _expected_metal_cross_evidence()
        or mapping != _expected_metal_mapping_evidence()
    ):
        raise RuntimeError("AES-128 Metal evidence ledger invocation gate failed")
    ...
    try:
        ...
        raise RuntimeError("AES-128 Metal evidence ledger consistency gate failed") from error
...
def run_metal_qualification(
    *,
    ...
    if wall_seconds > QUALIFICATION_METAL_WALL_CAP_SECONDS + 2:
        raise RuntimeError("AES-128 qualification exceeded its enforced Metal wall cap")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--build-dir",
        type=Path,
        default=Path(__file__).parents[1] / "build" / "aes128_metal_qualification",
    )
